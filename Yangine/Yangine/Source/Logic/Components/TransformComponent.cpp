...
bool yang::TransformComponent::Init(tinyxml2::XMLElement* pData)
{
	...
	m_scale = FVectorFromXML(pData->FirstChildElement("Scale"), { 1.f, 1.f });
...
void yang::TransformComponent::UpdateTransformMatrix()
{
	...
	m_transformMatrix.Rotate(m_rotationAngle, m_rotationPoint).Scale(m_scale, m_scalePoint).Translate(m_position);
}
